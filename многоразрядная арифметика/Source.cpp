...
bool ReadLong(ifstream &infile, separation_String &num, string &Eror) {
	...
	int chek;//переменная для проверк числа в файле
	numb_str++;
		infile >> buffer;
		if (buffer == "")
		{
		...
		if (chek == 0) {
			...
			switch (chek)
			{
			case 1: Eror = "в строке  "; Eror += std::to_string(numb_str); Eror += " неккоректное вещественное число \n"; return false;
			case 2: Eror = "в строке  "; Eror += std::to_string(numb_str); Eror += " больше 99 символов \n"; return false;
			}
		}
		return true;
		
	}else if (!infile){  Eror = "файл не открывается\n"; return false; }
}
int ChekLong(string num)//проверка числа. Я НЕ РАЗОБРАЛСЯ В МНОЖЕСТВАХ, ТАМ КАКАЯ ТО ЕБАЛА, ПО ЭТОМУ ТАК
{
	int chek =0;
	int point = 0;
	...
	if ((point == 0) || (point > 1)) return 1;// если запятая не одна, то выход
...
{
	EQLONG(A, B);
	PrintLong(A);// переворачиваем число A
	PrintLong(B);//переворачиваем число B
	char a1;
